...
import { access, copyFile, link, mkdtemp, readdir, rm, stat, unlink, } from 'node:fs/promises';
...
async function getUniqueProjectName(outputFolder, projectName) {
    const names = await readdir(outputFolder);
    let candidate = projectName;
    let suffix = 2;
    while (names.some((name) => new RegExp(`^${escapeExpression(candidate)} Part \\d+\\.mp4$`, 'i').test(name))) {
        candidate = `${projectName} (${suffix})`;
        suffix += 1;
    }
    return candidate;
}
async function removeOwnedTemporaryDirectory(outputFolder, directory) {
    const relative = path.relative(outputFolder, directory);
    const isOwned = relative.length > 0 &&
        !relative.startsWith('..') &&
...
async function promoteFile(source, destination) {
    try {
        ...
        if (code === 'EEXIST')
            throw mapFileError(error, 'OUTPUT_COLLISION');
