import type { FFmpegErrorCode } from './ffmpeg.types'

export class ServiceFailure extends Error {
  constructor(readonly code: FFmpegErrorCode, message: string) {
    super(message)
  }
}
...
export function classifyFfmpegFailure(stderr: string) {
  ...
  if (
    ...
    return new ServiceFailure(
      'UNSUPPORTED_MEDIA',
      'The media cannot be stream-copied to MP4.',
    )
  }
